...
import { Bell, Calendar, FileText, AlertCircle, CheckCircle, Clock, Image as ImageIcon } from 'lucide-react';
...
export default function AlumnoNotificacionesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        const avisosRes = await fetch(`${API_URL}/api/v1/avisos-escuela/`, {
      ...
      // Fetch actividades
      try {
        const actsRes = await fetch(`${API_URL}/api/v1/actividades-alumno/`, {
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
        });
        ...
        if (actsRes.ok) {
          ...
          acts.forEach((act: any) => {
            if (act.fecha_entrega) {
              ...
              const diasDiferencia = Math.ceil((fechaEntrega.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
              ...
              if (diasDiferencia >= 0 && diasDiferencia <= 7) {
                notifs.push({
                  ...
                  titulo: act.titulo,
                  mensaje: `Fecha de entrega: ${fechaEntrega.toLocaleDateString('es-AR')}`,
                  fecha: act.fecha_entrega,
                  leida: false,
                  materia: act.materia,
                });
              }
      ...
      // Fetch entregas corregidas
      try {
        const entsRes = await fetch(`${API_URL}/api/v1/entregas-alumno/`, {
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
        });
        ...
        if (entsRes.ok) {
          ...
          ents.forEach((ent: any) => {
            if (ent.nota !== undefined && ent.nota !== null) {
              notifs.push({
                id: ent.id,
                tipo: 'CORRECCION',
                titulo: ent.actividad_titulo || 'Actividad corregida',
                mensaje: `Nota: ${ent.nota}`,
                fecha: ent.updated_at || ent.created_at,
                leida: false,
                materia: ent.materia,
              });
            }
      ...
      if (notifs.length === 0) {
        notifs.push({
          ...
          mensaje: 'Explora tus cursos y materias en el menu.',
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500">Actualizaciones y avisos importantes</p>
      </div>
      ...
      {notificaciones.length === 0 ? (
        ...
        <div className="space-y-4">
          {notificaciones.map(notif => {
            ...
            return (
              <Card key={`${notif.tipo}-${notif.id}`} className={notif.leida ? 'opacity-60' : ''}>
                <CardContent className="pt-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {notif.materia && (
                        <p className="text-xs text-gray-400 mt-2">Materia: {notif.materia}</p>
                      )}
