  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500">Fechas de entrega y exámenes</p>
      </div>

      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <Button variant="outline" onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1))}>←</Button>
        ...
        <CardContent>
          <div className="grid grid-cols-7 gap-1">
            {DIAS_SEMANA.map(dia => (
              <div key={dia} className="text-center font-medium text-sm p-2 bg-gray-50">{dia}</div>
            ))}
            {days.map((day, index) => {
              const dayEvents = day ? getEventosForDay(day) : [];
              return (
                <div key={index} className={'min-h-[80px] border p-1 ' + (day ? 'bg-white' : 'bg-gray-100')}>
                  {day && (
                    <>
                      <div className="text-sm font-medium">{day}</div>
                      {dayEvents.map(e => (
                        <div key={e.id} className={'text-xs p-1 mt-1 rounded ' + (e.tipo === 'EXAMEN' ? 'bg-red-100 text-red-700' : e.tipo === 'ENTREGA' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700')}>
                          {e.titulo}
                        </div>
                      ))}
                    </>
